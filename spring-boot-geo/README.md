...
```lua
开始批量逆地理编码，共 121 个坐标，分 7 批处理
批次 1 成功，发送 20 个坐标，有效 18 条，过滤无效 2 条
批次 2 第1次调用失败 [20000]: INVALID_PARAMS
批次 2 第2次调用失败 [20000]: INVALID_PARAMS
批次 2 第3次调用失败 [20000]: INVALID_PARAMS
批次 3 成功，发送 20 个坐标，有效 20 条，过滤无效 0 条
批次 4 成功，发送 20 个坐标，有效 20 条，过滤无效 0 条
批次 5 成功，发送 20 个坐标，有效 20 条，过滤无效 0 条
批次 6 成功，发送 20 个坐标，有效 1 条，过滤无效 19 条
批次 7 调用成功返回空对象，跳过
批量逆地理编码完成，坐标数 121，返回结果数 79
```

注意结果区分：调用失败、返回内容但是不规范、直接返回空对象。
